...
class TestAssemblyAITranscriber:
    ...
    def test_init_with_env_key(self, mock_env_api_key, mock_aai_transcriber_cls, mock_aai_settings):
        ...
        assert mock_aai_settings.api_key == "test_env_key"
        mock_aai_transcriber_cls.assert_called_once()
        assert transcriber.prompt is None

    def test_init_with_explicit_key_and_prompt(self, mock_aai_transcriber_cls, mock_aai_settings):
        """Test initialization with explicit API key and prompt."""
        # pylint: disable=unused-argument
        transcriber = AssemblyAITranscriber(
            api_key="explicit_key", prompt="test prompt")
        assert transcriber.api_key == "explicit_key"
        assert mock_aai_settings.api_key == "explicit_key"
        assert transcriber.prompt == "test prompt"
    ...
    def test_transcribe_success(self, mock_config_cls, mock_env_api_key, mock_aai_transcriber_cls):
        ...
        # pylint: disable=unused-argument
        transcriber = AssemblyAITranscriber(prompt="custom prompt")
        ...
        assert result.conversation[1].timestamp == "00:01:05"
        assert result.conversation[1].person == "Speaker B"
        assert result.conversation[1].content == "World"
